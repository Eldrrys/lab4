...
bool check_input(char el, char next_el, int i) {
...
char* to_polish(char* input) {
	...
	for (int i = 0; i < len - 1; i++) {
		if (!check_input(input[i], input[i + 1], i)) {
			free(result);
			return error;
		}
